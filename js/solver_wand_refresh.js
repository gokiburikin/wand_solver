...
const WAND_MANA_RECHARGE_SPEED = 0;
...
let cast_projectile_count = 0;
let root_shot = null;
let first_shot = true;
let decks_tested = 0;
let failed_to_cast = false;
let zero_score = false;
//let permutations = {};
...
function move_hand_to_discarded(){
	console.log(force_stop_draws);
	for ( let action of hand ){
		if ( action.permanently_attached != true ){
			console.log("move",action.id,"from hand to discard");
...
function move_discarded_to_deck()
{
    ...
    for ( let action of discarded ){
		console.log("move",action.id,"from discard to deck");
        deck.push( action );
    }
	discarded = [];
}
...
function draw_action( instant_reload_if_empty )
{
	...
	if ( deck.length <= 0 ){
		if ( instant_reload_if_empty == true && force_stop_draws == false ) {
			console.log("draw action discard2deck")
	...
	if ( deck.length > 0 ) {
		...
		if ( action_mana_required > mana ) {
			//console.log("failed to cast",action )
			failed_to_cast = true;	
            discarded.push( action );
			return false;
        }
...
function add_projectile( entity_filename ){
	cast_projectile_count = cast_projectile_count + 1;
}
	...
	},
	{
		...
		action 		: function(){
			...
			if ( force_stop_draws == false ) {
				...
				console.log("reset action discard2deck")
				move_discarded_to_deck();
				order_deck();
			}
		},
...
for ( let action of actions ) {
	...
	action.action = function(...args){
		...
		if ( c_cache.length > 30 || called_actions.length > 30){
			reloading = true;
			force_stop_draws = true;
			zero_score = true;
		} else {
...
function score_function( damage, speed_multiplier, lifetime_add, mana, projectiles, fxcrit_chance, actions, accelerative_homing, homing_rotates, resets ){
	if ( zero_score == true ){ return 0; };
    let score = 0;
...
function register_action( state ){
	console.log("reigster action");
    state.reload_time = current_reload_time;
	console.log( called_actions.slice() );
	console.log( "deck",deck.flatMap( v=>v.id) );
	console.log( "hand",hand.flatMap( v=>v.id) );
	console.log( "discard",discarded.flatMap( v=>v.id) );
	called_actions = [];
	c_cache.push( state );
    return state;
}
...
function score_cast(){
	let meaningful_projectiles = cast_projectile_count - ( c.useless_projectiles ?? 0 );
	let deck_score = 0;
	for ( let c of c_cache ){
		let score = score_function( c.damage_projectile_add * 25, c.speed_multiplier, c.lifetime_add, total_shot_mana, meaningful_projectiles, c.fxcrit_chance, last_deck, c.accelerative_homing, c.homing_rotates, c.resets );
    	deck_score += score;
	}
...
function _handle_reload(){
	...
	console.log( false, reloading,deck.length,start_reload );
	if ( reloading == false && ( deck.length <= 0 || start_reload == true ) ) {
		decks_tested++;
		console.log("	handle reload discard2deck")
		move_discarded_to_deck();
		order_deck();
	...
	console.log("handle reload complete");
	return mana;
}

function Update(){
console.log(OPTIONS.DecksToTest )

    if ( wand_deck.length > 0 ) {
        for ( let i = 0; i < OPTIONS.SolverIterations; i++ ){
			if ( OPTIONS.DecksToTest == -1 || decks_tested < OPTIONS.DecksToTest) {
				_start_shot( mana );
				draw_action( true );
				register_action( c );
				_handle_reload();
				if ( force_stop_draws || failed_to_cast ){
					reloading = false;
					start_reload = true;
					_handle_reload()
				}
...
onmessage = function( e ){
    if ( e.data.type === "deck" ){
		...
		OPTIONS.ScoreLifetime = e.data.score_lifetime;
		OPTIONS.ImprovementThreshold = e.data.improvement_threshold;
		OPTIONS.DecksToTest = e.data.decks_to_test;
	}
}
